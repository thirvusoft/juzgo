...
def get_result(filters, tds_docs, tds_accounts, tax_category_map, journal_entry_party_map, invoice_net_total_map):
    ...
    for name, details in gle_map.items():
        ...
        for entry in details:
            supplier = entry.get('party') or entry.get('against')
            ...
            if voucher_type == "Sales Invoice":
                sales_invoice = frappe.get_doc("Sales Invoice", entry.get('voucher_no'))
            ...
            if entry.get('account') in tds_accounts:
                tds_deducted += entry.get('credit', 0) - entry.get('debit', 0)
...
def get_tds_docs(filters):
    ...
    if filters.get("customer"):
        query_filters.update({"party": filters.get("customer")})


    tds_docs = frappe.get_all(
        "GL Entry",
        filters={"voucher_type": "Sales Invoice", "party": filters.get("customer"),   "posting_date": ("between", [filters.get("from_date"), filters.get("to_date")])},
