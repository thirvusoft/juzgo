...
import json
@frappe.whitelist()
def add_member_details(row,name):
    list = []
    row = json.loads(row)
    ...
    if not table_doc:frappe.throw("Check List Not Found for Gender "+row.get('gender')+" and age "+str(row.get('age')))
        ...
        for i in check_list_items:
            list.append({'members_name':row.get('members_name'),'age':row.get('age'),'gender':row.get('gender'),'check_list_name':i.check_list_name,'family_member_details_name':row.get('member_row_id'),'check':0})
        return(list)
@frappe.whitelist()
